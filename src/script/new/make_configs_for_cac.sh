...
function configure_prefs_js() {
  ...
  # add "listModel.append(new CustomAccentColors('Custom (Default)', 'custom'));" under "listModel.append(new CustomAccentColors('Blue', 'blue'));"
  sed -i '/listModel.append(new CustomAccentColors('\''Blue'\'', '\''blue'\''));/a listModel.append(new CustomAccentColors('\''Custom (Default)'\'', '\''custom'\''));' "${custom_acc_color_prefs_js_path}"
  # add 2 tabs before "listModel.append(new CustomAccentColors('Custom (Default)', 'custom'));" to match the other lines
  sed -i 's/listModel.append(new CustomAccentColors('\''Custom (Default)'\'', '\''custom'\''));/\t\tlistModel.append(new CustomAccentColors('\''Custom (Default)'\'', '\''custom'\''));/g' "${custom_acc_color_prefs_js_path}"
}
